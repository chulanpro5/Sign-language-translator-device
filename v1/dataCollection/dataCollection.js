...
function getData() {
    ...
    if (dem == 5) {
        ...
        console.log('jointTrain');
        console.log(jointTrain);

        console.log(jointTrain[0][0][0]);

        for (let i = 0; i <= 1; i++)
            for (let j = 0; j <= 44; j++)
                for (let k = 0; k <= 4; k++) {
                    if (jointTrain[i][j][k] === undefined)
                        inputsTrain.push(0);
                    else inputsTrain.push(jointTrain[i][j][k]);
                }
    ...
    } else if (dem < 5) setTimeout(getData, 600);
}
